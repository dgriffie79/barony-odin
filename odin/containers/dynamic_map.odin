...
dynarrstrpair_value_free :: proc(p: rawptr) {
	barony_dynamic_array_elem_destroy((^Raw_Dynamic_Array)(p), size_of(DynamicStringPair_t), Kind_StringPair)
}
dynarrstrpair_value_copy :: proc(dst: rawptr, src: rawptr) {
	barony_dynamic_array_elem_copy((^Raw_Dynamic_Array)(dst), (^Raw_Dynamic_Array)(src), size_of(DynamicStringPair_t), Kind_StringPair)
}
...
// map[string]CompendiumAchievementsDisplay (achievementsBookDisplay).
// 48B: Raw_Dynamic_Array of DynamicArrayStr + 2 ints.
CompendiumAchievementsDisplay_t :: struct {
	pages:       Raw_Dynamic_Array, // of Raw_Dynamic_Array of string
	currentPage: i32,
	numHidden:   i32,
}
#assert(size_of(CompendiumAchievementsDisplay_t) == 48)

compendium_achievements_display_free :: proc(p: rawptr) {
	v := (^CompendiumAchievementsDisplay_t)(p)
	barony_dynamic_array_elem_destroy(&v.pages, size_of(Raw_Dynamic_Array), Kind_DynArrayStrArray)
}
compendium_achievements_display_copy :: proc(dst: rawptr, src: rawptr) {
	d := (^CompendiumAchievementsDisplay_t)(dst)
	s := (^CompendiumAchievementsDisplay_t)(src)
	d^ = CompendiumAchievementsDisplay_t{}
	d.currentPage = s.currentPage
	d.numHidden = s.numHidden
	barony_dynamic_array_elem_copy(&d.pages, &s.pages, size_of(Raw_Dynamic_Array), Kind_DynArrayStrArray)
}

// map[string]CompendiumMapTiles_t (compendiumObjectMapTiles).
// 56B: POD CompendiumMap_t + Raw_Dynamic_Array of i32.
CompendiumMap_t :: struct {
	width:   u32,
	height:  u32,
	ceiling: u32,
}
#assert(size_of(CompendiumMap_t) == 12)
CompendiumMapTiles_t :: struct {
	first:  CompendiumMap_t,
	second: Raw_Dynamic_Array, // of i32
}
#assert(size_of(CompendiumMapTiles_t) == 56)

compendium_map_tiles_free :: proc(p: rawptr) {
	v := (^CompendiumMapTiles_t)(p)
	barony_dynamic_array_elem_destroy(&v.second, size_of(i32), Kind_POD)
}
compendium_map_tiles_copy :: proc(dst: rawptr, src: rawptr) {
	d := (^CompendiumMapTiles_t)(dst)
	s := (^CompendiumMapTiles_t)(src)
	d^ = CompendiumMapTiles_t{}
	d.first = s.first
	barony_dynamic_array_elem_copy(&d.second, &s.second, size_of(i32), Kind_POD)
}

// map[string]ObjectLimbs_t (compendiumObjectLimbs).
// 216B: 2 POD CompendiumView_t + raw DynamicArray of Entity (POD, byte moves).
CompendiumView_t :: struct {
	ang:             f64,
	vang:            f64,
	pan:             f64,
	zoom:            f64,
...
#assert(size_of(CompendiumView_t) == 88)
ObjectLimbs_t :: struct {
	baseCamera:    CompendiumView_t,
	currentCamera: CompendiumView_t,
	entities:      Raw_Dynamic_Array, // of Entity (POD)
}
#assert(size_of(ObjectLimbs_t) == 216)

object_limbs_free :: proc(p: rawptr) {
	v := (^ObjectLimbs_t)(p)
	barony_dynamic_array_destroy(&v.entities)
}
object_limbs_copy :: proc(dst: rawptr, src: rawptr) {
	d := (^ObjectLimbs_t)(dst)
	s := (^ObjectLimbs_t)(src)
	d^ = ObjectLimbs_t{}
	d.baseCamera = s.baseCamera
	d.currentCamera = s.currentCamera
...
// map[string]Codex_t (codex).
// 344B: 2 strings + 4 str arrays + 1 u32 array + POD view + 3 ints + bool.
Codex_t :: struct {
	modelIndex:         i32,
	imagePath:          string,
	renderedImagePaths: Raw_Dynamic_Array, // of string
	blurb:              Raw_Dynamic_Array, // of string
	linesToHighlight:   Raw_Dynamic_Array, // of u32
	details:            Raw_Dynamic_Array, // of string
	models:             Raw_Dynamic_Array, // of string
	featureImg:         string,
	id:                 i32,
	view:               CompendiumView_t,
	lorePoints:         i32,
	enableTutorial:     bool,
}
#assert(size_of(Codex_t) == 344)

codex_t_free :: proc(p: rawptr) {
	v := (^Codex_t)(p)
	dynamic_string_free_elem(rawptr(&v.imagePath))
	dynamic_string_free_elem(rawptr(&v.featureImg))
	barony_dynamic_array_elem_destroy(&v.renderedImagePaths, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_destroy(&v.blurb, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_destroy(&v.details, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_destroy(&v.models, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_destroy(&v.linesToHighlight, size_of(u32), Kind_POD)
}
codex_t_copy :: proc(dst: rawptr, src: rawptr) {
	d := (^Codex_t)(dst)
	s := (^Codex_t)(src)
	d^ = Codex_t{}
	d.modelIndex = s.modelIndex
	...
	d.view = s.view
	d.lorePoints = s.lorePoints
	d.enableTutorial = s.enableTutorial
	dynamic_string_copy_elem(rawptr(&d.imagePath), rawptr(&s.imagePath))
	dynamic_string_copy_elem(rawptr(&d.featureImg), rawptr(&s.featureImg))
	barony_dynamic_array_elem_copy(&d.renderedImagePaths, &s.renderedImagePaths, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_copy(&d.blurb, &s.blurb, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_copy(&d.details, &s.details, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_copy(&d.models, &s.models, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_copy(&d.linesToHighlight, &s.linesToHighlight, size_of(u32), Kind_POD)
...
// map[string]ItemsCodex_t (items + magic).
// 112B: 1 string + 1 str array + 1 CodexItem_t array + 2 ints.
ItemsCodex_t :: struct {
	modelIndex:        i32,
	imagePath:         string,
	blurb:             Raw_Dynamic_Array, // of string
	items_in_category: Raw_Dynamic_Array, // of CodexItem_t
	lorePoints:        i32,
}
#assert(size_of(ItemsCodex_t) == 112)

items_codex_free :: proc(p: rawptr) {
	v := (^ItemsCodex_t)(p)
	dynamic_string_free_elem(rawptr(&v.imagePath))
	barony_dynamic_array_elem_destroy(&v.blurb, size_of(DynamicString), Kind_DynamicString)
	barony_dynamic_array_elem_destroy(&v.items_in_category, size_of(CodexItem_t), Kind_CodexItem)
}
items_codex_copy :: proc(dst: rawptr, src: rawptr) {
	d := (^ItemsCodex_t)(dst)
	s := (^ItemsCodex_t)(src)
	d^ = ItemsCodex_t{}
	d.modelIndex = s.modelIndex
...
// IntPair_t — 8B POD (pair<int,int>). Value for entityAchievementsToProcess
// inner map (ticks remaining, counter).
IntPair_t :: struct {
	first:  i32,
	second: i32,
}
...
i32_map_intpair_value_copy :: proc(dst: rawptr, src: rawptr) {
	m := transmute(^map[[4]byte]IntPair_t)(dst)
	s := transmute(^map[[4]byte]IntPair_t)(src)
	if m^ != nil {
		delete(m^)
	}
	m^ = nil
...
value_ops_for :: proc(kind: i32) -> Value_Ops {
	...
	case .MK_ArrayStringPair:
		return Value_Ops{ free = dynarrstrpair_value_free, copy = dynarrstrpair_value_copy }
	case .MK_CompendiumAchievementsDisplay:
		return Value_Ops{ free = compendium_achievements_display_free, copy = compendium_achievements_display_copy }
	case .MK_CompendiumMapTiles:
	...
	case .MK_IntPair:
		return Value_Ops{}
	case .MK_I32MapIntPair:
		return Value_Ops{ free = i32_map_intpair_value_free, copy = i32_map_intpair_value_copy }
	case .MK_Lootbag:
